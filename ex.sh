...
# CIL CONFIG
GPU=5
LOG=logs/ocs_result_cifar100_dis_seed3.out
SEEDS="3"
NOTE="ocs_result_cifar100_dis"
SIGMA=0
...
if [ "$DATASET" == "cifar10" ]; then
    ...
    LAMBDA_FP_REPLAY=0.1 LAMBDA_FP=0.005 DER_ALPHA=0.3 DER_BETA=0.9
    ...
    LAMBDA_FP_REPLAY=1.2 LAMBDA_FP=0.005 DER_ALPHA=0.3 DER_BETA=1.2

elif [ "$DATASET" == "tinyimagenet" ]; then
    MEM_SIZE=4000 NUM_FUTURE_CLASS=20
...
do
    ...
    CUDA_VISIBLE_DEVICES=$GPU nohup python main_new.py > $LOG 2>&1 --mode $MODE --residual_strategy $RESIDUAL_STRATEGY $USE_NECK_FORWARD --moco_coeff $MOCO_COEFF \
    ...
    $IMP_UPDATE_PERIOD $USE_AMP --n_worker $N_WORKER --future_steps $FUTURE_STEPS --eval_n_worker $EVAL_N_WORKER --eval_batch_size $EVAL_BATCH_SIZE --n_tasks=5 &
done
